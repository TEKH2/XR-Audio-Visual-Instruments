...
namespace EXP.Painter
{
    ...
    public class StrokeRenderer_ParticleSystem : StrokeRenderer
    {
        ...
        public override void Initialize(BrushStroke s, int layer, Color col)
        {
        ...
        {            
            ...
            if (_Stroke == null || _Stroke.SpacedStrokeNodes.Count < 2) return;

            base.DrawStroke(forceRedraw);
          
            if (_PopulatedMatrixIndex != _Stroke.SpacedStrokeNodes.Count)
            {                  
                m_PSys.Clear();
                _PopulatedMatrixIndex = 0;

                List<StrokeNode> nodes = _Stroke.SpacedStrokeNodes;
                for (int i = _PopulatedMatrixIndex; i < nodes.Count; i++)
                {
                    EmitAtStrokeNode(nodes[i], i);
                }

                _PopulatedMatrixIndex = nodes.Count;
            }
            else if (forceRedraw)
            {
        ...
        void UpdatePartilcePositions()
        {
            ...
            {
                m_Particles[i].position = _Stroke.SpacedStrokeNodes[i].ModifiedPos;
