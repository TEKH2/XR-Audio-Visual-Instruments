...
{
    public class StrokeRenderer_TubeMoving : StrokeRenderer
    {
        #region VARIABLES
        ...
        bool _Play = true;
        [Range(0,1)]
        public float _PlayNorm = 0;
        [Range(0, 1)]
        public float _PlayLength = .2f;
        private void Update()
        {
            if(_Play)
            {
                _PlayNorm = (Time.time * .3f) % 1;
            }

            GenerateMeshFromStrokeNodes(_Stroke, _PlayNorm, _PlayLength);
        }
        ...
        void GenerateMeshFromStrokeNodes(BrushStroke stroke, float startNorm = 0, float normLength = 1)
        {
            // Clear mesh
            if (m_Mesh != null)
                m_Mesh.Clear();
            ...
            // Create new lists
            if (_PopulatedMatrixIndex == 0 || normLength != 0)
            {
            ...
            // Calc segment count and start and end nodes       
            int segmentCount = (int)(stroke.SpacedStrokeNodes.Count * normLength);

            // If not drawing the whole length of the tube then don't only fill mesh details ffrom the 
            int startNode = _PopulatedMatrixIndex;
            if (normLength != 1)
                startNode = (int)(stroke.SpacedStrokeNodes.Count * startNorm);
            
            float endNorm = Mathf.Clamp01(startNorm + normLength);
            int endNode =  (int)(stroke.SpacedStrokeNodes.Count * endNorm);

            List<StrokeNode> strokeNodes = stroke.SpacedStrokeNodes;
            int vertCount = 2 * segmentCount * m_RadialFaces;
            int vertIndex = 0;
            ...
            {
                float normVal = (float)segmentIndex / (segmentCount - 1);    // 0.0 -> 1.0                
                ...
                if (segmentIndex > 0)
                {
                    ...
                    int baseIndex = segmentIndex * m_RadialFaces;
                    for (int j = 0; j < m_RadialFaces; j++)
                    {
                        int index0;
                        int index1;
                        ...
                        highestIndex = Mathf.Max(highestIndex,index0, index1 , index2 , index3 );
