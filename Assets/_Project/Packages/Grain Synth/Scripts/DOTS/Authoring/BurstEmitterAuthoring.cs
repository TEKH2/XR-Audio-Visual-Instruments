...
public class BurstEmissionProps
{
    ...
    [Range(0.0f, 2.0f)]
    [SerializeField]
    public float _VolumeStart = 1;          // from 0 > 1
    [Range(0.0f, 1.0f)]
    [SerializeField]
    ...
    public float VolumeStart
    {
        get
        {
            return Mathf.Clamp(_VolumeStart + Random.Range(-_VolumeRandom, _VolumeRandom), 0f, 3f);
        }
        set
        {
            _VolumeStart = Mathf.Clamp(value, 0f, 3f);
        }
    }
