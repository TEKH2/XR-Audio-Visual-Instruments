...
public class StrokeNode
{
    ...
    public StrokeNode(Transform brushTipT, Transform strokeTransform, StrokeNode prevNode = null)
    {
        ...
        m_OriginalPos = brushTipT.localPosition;
        //m_OriginalRot = transform.localRotation;
        m_OriginalScale = brushTipT.localScale;

        // Determine rotation from tanget(facing), normal and binormal
        ...
        if (prevNode != null)
            m_OriginalRot = TransformExtensions.SmoothRotation(brushTipT.position, prevNode.m_OriginalPos, prevNode.m_OriginalRot);
        else
            m_OriginalRot = Quaternion.LookRotation(tanget, normal);
